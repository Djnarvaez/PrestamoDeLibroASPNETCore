...
namespace WEBBook.Repository
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        public async Task<ModelStateError> CreateAsync(string url, T entity)
        {
            ...
            request.Content = new StringContent(JsonConvert.SerializeObject(entity), Encoding.UTF8, Util.ContentType);
            ...
            if (response.StatusCode == HttpStatusCode.NotFound)
            {
                return new ModelStateError()
                {
                    Response = new Response()
                    {
                        Errors = new List<Errors>()
                        {
                            new Errors(){ErrorMessage = "No se encontró el recurso que desea eliminar" }
                        }
                    }
                };
            }
            ...
            if (response.StatusCode == HttpStatusCode.BadRequest)
            {
                return new ModelStateError()
                {
                    Response = new Response()
                    {
                        Errors = new List<Errors>()
                        {
                            new Errors(){ErrorMessage = "No se encontró el recurso que desea actualizar" }
                        }
                    }
                };
            }
